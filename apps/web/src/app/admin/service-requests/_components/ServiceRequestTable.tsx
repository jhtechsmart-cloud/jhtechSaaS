...
export function ServiceRequestTable({ items }: { items: ServiceRequestListRow[] }) {
    ...
    <div className="flex flex-col gap-3">
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center gap-2 rounded-md border border-border bg-surface p-8">
          ...
          <button onClick={() => { setQ(""); setStatusFilter("all"); }} className="text-small text-accent underline">
            필터 초기화
          </button>
        </div>
      ) : (
          ...
          <table className="w-full border-collapse text-body">
            <thead>
              <tr className="border-b border-border text-left text-small text-muted">
                ...
                <th className="py-2 pr-4 font-medium">증상</th>
            ...
            <tbody>
              {filtered.map((it) => (
                ...
                >
                  ...
                  <td className="max-w-xs py-2 pr-4">
                    <span className="block max-w-xs truncate text-muted">{it.symptom}</span>
                  </td>
