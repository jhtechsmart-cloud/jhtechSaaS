...
import { updateSupplyRequestStatus } from "@/lib/supply-requests/admin-actions";
import { STATUS_META } from "@/lib/request-status";

const TERMINAL = new Set<SupplyRequestStatus>(["done", "canceled"]);

export function StatusControl({ id, current }: { id: string; current: SupplyRequestStatus }) {
  const router = useRouter();
  const [status, setStatus] = useState<SupplyRequestStatus>(current);
  ...
  function apply() {
    ...
    startTransition(async () => {
      const res = await updateSupplyRequestStatus(id, status);
  ...
  return (
    <div className="flex items-center gap-2">
      <select
        value={status}
        onChange={(e) => setStatus(e.target.value as SupplyRequestStatus)}
