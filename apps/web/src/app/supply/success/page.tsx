...
import { supSeqNoSchema } from "@/lib/supply-requests/schema";

// no(접수번호) 없이/형식위반 직접 진입 시 홈으로(새로고침·북마크 안전).
export default async function SupplySuccessPage({
  searchParams,
}: {
  searchParams: Promise<{ no?: string; assignee?: string }>;
}) {
  const { no, assignee } = await searchParams;
  if (!no || !supSeqNoSchema.safeParse(no).success) redirect("/");
