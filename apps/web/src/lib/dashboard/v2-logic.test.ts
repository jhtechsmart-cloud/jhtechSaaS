...
describe("buildTwoWeekDays — 이번 주 일요일부터 14일", () => {
  test("금요일(2026-06-12) 기준: 6/7(일)~6/20(토), 오늘·지난날 표식", () => {
    ...
    expect(days[0].date).toBe("2026-06-07"); // 이번 주 일요일
    expect(days[13].date).toBe("2026-06-20"); // 다음 주 토요일
  ...
  test("일요일 기준: 그날이 주 시작(주 시작=일)", () => {
    ...
    expect(days[0]).toMatchObject({ date: "2026-06-14", isToday: true }); // 일요일=주 시작
  ...
  test("토요일(2026-06-13) 기준: 최대 역방향 오프셋(-6) + dow 0=일…6=토", () => {
    ...
    expect(days[6]).toMatchObject({ date: "2026-06-13", isToday: true, dow: 6 }); // 토
...
describe("buildCalendarDays — 뷰별 그리드(일요일 시작)", () => {
  ...
  test("twoweek: 14일(이번 주 일 ~ 다음 주 토)", () => {
