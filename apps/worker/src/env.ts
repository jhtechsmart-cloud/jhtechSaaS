...
const envSchema = z.object({
  ...
  // 승인 요청 알림(#285): 기사 프로필에 하이웍스 ID가 없을 때 쓰는 발신자 폴백(선택). 둘 다 없으면 알림 잡 실패.
  HIWORKS_NOTICE_FALLBACK_USER: z.string().min(1).optional(),
  // 알림 본문의 상세 링크 호스트. 미설정 시 운영 관리자 콘솔.
  ADMIN_SITE_URL: z.string().url().default("https://admin.jhtech.co.kr"),
