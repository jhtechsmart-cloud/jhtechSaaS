...
### Added
- **관리자 사이드바 배지·알림 자동 갱신(60초 폴링)** — 콘솔을 켜둔 상태에서 새 견적요청·A/S·소모품 신청이 들어오면, 수동 새로고침 없이 최대 1분 안에 사이드바 배지(견적·A/S·소모품)와 종 알림에 반영. `BadgePoller`(클라)가 60초마다 `router.refresh()`로 현재 라우트 서버 컴포넌트를 재실행(기존 카운트 쿼리·RLS 그대로 재사용). 탭이 보이지 않을 땐 폴링을 멈추고, 다시 활성화되면 즉시 1회 갱신. (App Router 레이아웃이 클라 내비로는 재실행되지 않아 새 의뢰가 새로고침 전까지 안 보이던 문제 해소.)

## [0.13.2.5] - 2026-06-16

### Fixed
- **관리자 페이지 하단 여백 보강** — 저장/취소 버튼이 화면 맨 아래에 바짝 붙어 답답하던 문제. 일반 관리자 페이지(`ConsoleMain`)와 의뢰관리 견적 상세(자체 스크롤 칸)에 하단 64px 여백(`pb-16`)을 추가해 버튼 아래에 숨 쉴 공간을 확보. 의뢰관리는 고정 높이 2분할이라 상위 래퍼가 아니라 상세 스크롤 칸에서 처리(상위에 주면 2분할이 잘림). 고객 포털은 기존 `py-10` + 푸터로 여백이 충분해 변경 없음.
...
### Changed
- **데모 일정 색을 틸 → 보라로 변경** — 데모(틸)와 견적(파인 초록)이 날짜 배지에서 구분이 잘 안 돼, 데모를 보라(`#7C5CD3`)로 분리. 대시보드 2주 캘린더 칩·날짜 배지·범례, 데모예약 월간 캘린더 dot·범례·타임라인·요약 블록까지 전부 일괄 반영. 이제 5색이 또렷이 구분(견적=파인 · A/S=코랄 · 소모품=라임 · 데모=보라 · 납품=파랑).
- **대시보드 2주 일정 캘린더 날짜 칸 높이 +20%** (96→115px) — 한 칸에 들어가는 일정이 더 여유 있게 보임.

### Added
- **데모예약 페이지 캘린더 아래 "이번 달 예약" 리스트** — 표시 중인 달의 데모 예약을 시작시각 순으로 모아 보여줌(고객·장비·시간, 완료 예약은 회색 + "완료" 태그). 행을 누르면 그 날짜로 타임라인 이동, 선택일은 보라 테두리로 강조.

## [0.13.2.2] - 2026-06-13

### Fixed
- **사이드바 접힘 시 아이콘 정렬 수정** — 접었다 펼 때 아이콘이 좌우로 튀던 흔들림, 접힘 상태에서 현재 위치 표시(동그란 배경)가 아이콘과 어긋나던 문제, 아이콘이 왼쪽으로 쏠리던 문제를 모두 해결. 아이콘을 고정 40px 레일에 중앙 배치해 펼침/접힘 무관하게 위치를 고정(측정값 펼침·접힘 모두 33px로 동일)하고, 접힘 시 항목 배경이 아이콘을 정확히 감싸는 정원이 되도록 함. 하단 프로필 아바타도 같은 방식으로 중앙 정렬.

## [0.13.2.1] - 2026-06-13

### Changed
- **대시보드 날짜·시간 배지를 항목 유형별 5색으로 구분** — 이번 달 신청·최근 활동·일정 레일의 날짜 음영 배지에 캘린더와 같은 색 언어 적용(견적=민트/파인 · A/S=코랄 · 소모품=라임 · 데모=틸 · 납품=파랑). 한눈에 어떤 종류의 건인지 구분. 배지는 글자에 맞춘 고정폭(단일시간 목록은 좁게, 시간 범위 일정은 넓게)으로 제목 시작 위치를 맞추고, 글씨 중앙정렬 + 모서리를 덜 둥글게(rounded-sm) 다듬음.
- **2주 일정 캘린더·주간 활동을 일요일 시작으로 변경** — 일반 달력처럼 일~토 순서로 표시.
- **주간 활동 날짜 라벨을 하단에 고정** — 건수가 많든 적든 날짜·요일 줄 위치가 흔들리지 않도록 블록 영역을 고정 높이로, 요일별 세로 구분선(레인)으로 하루하루를 명확히 분리. 날짜는 위·요일은 아래 2줄로.
- **본문 배경을 더 밝게(#FBFDFC) + 카드 그림자 강조** — 밝아진 배경에서 카드가 또렷하게 떠 보이도록 파인 그린 틴트 그림자를 한 단계 진하게.

### Fixed
- 날짜 배지 시간 줄과 주말 요일 라벨의 색 대비를 AA 기준(4.5:1)에 맞게 조정(작은 글씨 가독성).
...
### Changed
- **대시보드 시각 다듬기(피드백 반영)** — ① 2주 캘린더를 일자별 떨어진 박스에서 **일반 달력 형태**(요일 헤더 + 연속 셀 hairline 그리드)로 교체, 헤더에 연·월 표시(두 달에 걸치면 "6월–7월", 매월 1일 셀은 "7/1") + 오늘 셀 "오늘" 마커. ② 데모·납품 일정/이번 달 신청/최근 활동의 날짜·시간을 좌측 라인선 대신 **음영 배지**로(데모=민트·납품=파랑 틴트, 신청·활동=중립 음영) 내용과 구분. ③ KPI 4장 값 굵기 완화(extrabold→bold) + 카드에 옅은 대각 그라데이션.

## [0.13.1.0] - 2026-06-12

### Changed
- **대시보드 v2 개편 — "현황 + 2주 일정" 중심 재구성.**
  - **KPI 4장**: 처리 대기(견적 미배정+A/S 미처리+소모품 대기 합산, 1건 이상이면 코랄 경고) / 진행 중 견적(건수+최신 견적 합계 금액) / 이번 주 데모·납품(건수+데모센터 가동률 %) / 전체 고객(+이번 달 신규).
  - **2주 캘린더(전체 폭)**: "이번 주/다음 주" 구분선 2줄, 이벤트 5종 색 칩(견적=파인·A/S=코랄·소모품=라임·데모=틸·납품=파랑, 좌측 보더+옅은 배경), 지난 요일 흐림·오늘 민트 하이라이트, 시간 있는 이벤트는 "HH:mm" 접두, 클릭=해당 레코드 이동.
  - **견적 파이프라인(세로 행)**: 단계명+비율 바+건수, 행 클릭=견적 목록 필터, 발송 후 7일 경과 건 코랄 경고 노트.
  - **주간 활동(단위 블록)**: 요일별 블록 1개=1건 스택(상단 합계, hover 툴팁, 12건 초과 "+N").
  - **우측 레일**: 다가오는 데모·납품 5건 + 이번 달 신청 — 날짜 위/시간 아래 2줄 공용 형식("6/12 (금)" / "14:00–15:30"), 최근 활동도 동일 형식.
  - 역할 분기: 영업은 RLS 스코프(본인 담당) 그대로 — 라벨 "내 담당", 관리자는 "전체".
  - 콘솔 본문 최대 폭 1320→1180px(자체 사이드 패널 화면은 전체 폭 예외 유지). 구 대시보드 컴포넌트(액션큐·도넛·참조 카운트) 제거.

## [0.13.0.0] - 2026-06-12

### Added
- **데모예약 신규 기능** — 데모센터(1곳) 예약을 시스템으로 관리. 사이드바 '업무 > 데모예약' 메뉴 추가.
  - **예약 관리 페이지**(`/admin/demo-reservations`): 좌측 월간 캘린더(데모 예약일=틸 점·납품일=파랑 점, 날짜 클릭=일정 전환·URL 보존) + 우측 선택일 타임라인(09:00–18:00, 장비명/시간/고객·방문자·연락처·담당자, 블록 클릭=상세·취소).
  - **예약 등록 페이지**: 고객(기존 고객 검색 또는 미등록 직접 입력)·데모 장비·방문자·소요시간(30/60/90/120분) + **15분 단위 시작시간 슬롯 그리드**(점유 시간=취소선 비활성, 선택 범위=민트 하이라이트, 겹치면 경고 배너+저장 차단). 우측에 해당일 기존 예약 요약.
  - **중복 예약 원천 차단**: UI 검증과 별개로 DB EXCLUDE 제약이 최후 방어선 — 동시에 두 명이 같은 시간을 저장해도 한 건만 성공하고 나머지는 "방금 다른 예약이 등록되었습니다" 안내(동시성 db-test로 증명). 취소된 예약 시간은 재사용 가능.
  - 권한: 조회=전 직원, 등록·취소=영업·관리자(`demo_reservations.write`, 영업 프리셋 포함).
- **견적 납품 일정** — 발행(issued)된 견적에 납품일·시각 입력(요약 패널). 데모예약 캘린더·대시보드 일정의 '납품' 이벤트 소스.
...
### Changed
- **전체 테마 교체 — 라이트 민트·파인 그린** — 진한 네이비 사이드바·회청색 톤을 밝은 민트 셸로 전면 교체(레이아웃·기능 무변경). 사이드바는 페이지 배경과 같은 라이트 톤 + 섹션 라벨(업무/카탈로그/관리) + 파인 그린 활성 pill, 폰트 Pretendard 단일(헤딩 extrabold), 버튼·칩·인풋 pill, 그림자 1종 통일. 견적 화면의 네이비 히어로·QUOTE SUMMARY는 흰 카드+파인 헤딩(합계는 민트 강조)으로, 진한 블록은 사이드바 로고·고객 주 연락처 카드 2곳만 유지. 상태 배지는 의미 그대로 3톤(미처리=코랄·중립=뉴트럴·완료/발송=민트)으로, KPI 카드에 컬러 게이지 바. 접근성: 흐린 텍스트·코랄 텍스트를 WCAG AA 통과 값으로 보정, 포커스 링 시인성 강화.
