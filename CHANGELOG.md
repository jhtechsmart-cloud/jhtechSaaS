...
### Added
- **고객 목록 데이터 테이블 개편** — 1,270곳에서 원하는 거래처를 바로 찾도록 전면 재구성. 통합 검색(업체명·사업자번호·대표자·담당자·전화·장부번호, **하이픈 무시**, `/` 단축키), KPI 빠른 필터 4종(전체/거래중/미배정/최근 30일 활동 — 클릭=필터), 지역·담당영업 필터, 정렬(업체명·지역·최근활동), 페이지네이션(25/50/100), 검색어 하이라이트, CSV 내보내기(현재 필터 유지·수식 주입 중화). 검색·필터·정렬·페이지 전부 서버 처리 + 주소창 URL로 상태 보존(새로고침·공유·뒤로가기 복원). 거래현황(견적·장비·AS 건수)과 최근 활동(상대시간)을 행에서 바로 확인.

## [0.12.18.0] - 2026-06-12

### Added
- **고객 수정 폼 개편** — 13개 필드 1열 나열을 상세 페이지와 같은 그룹 카드(기본 정보/연락처/사업장/장부·회계 + 메모·보유장비) 2열 그리드로 재구성(860px 이하 1열). 헤더에 수정 중인 업체명·장부번호 표시 + 상세로 돌아가기. **변경 추적**: 수정한 필드는 amber 하이라이트, 하단 고정 저장 바에 "N개 항목 변경됨·필드명" 표시(변경 없으면 저장 비활성, 취소=원복). 저장하지 않고 떠나면(새로고침·뒤로가기) 확인 다이얼로그. 사업자번호·전화는 숫자만 쳐도 자동 하이픈. 저장 성공 시 상세 복귀+토스트.

## [0.12.17.1] - 2026-06-12

### Fixed
- **고객 상세 긴 항목 잘림** — 주소(사업장)·실제주소1/2·장부명·업종(종목)처럼 긴 값이 한 줄 말줄임으로 잘려 전체 내용을 볼 수 없던 문제. 긴 텍스트 필드는 라벨 아래 전체 폭으로 여러 줄 표시하고, 한 줄 유지 필드는 마우스를 올리면 전문이 뜨도록 개선.
...
### Added
- **거래처 1,270곳 이관 준비** — 회계 프로그램 엑셀(1,311행)을 고객 DB로 옮기는 파이프라인. 같은 회사의 분리 장부((장비)·(NEW) 등) 30쌍은 한 고객으로 병합(구 장부는 메모에 보존), "일반고객" 자리표시 행 제외, 사업자번호 숫자 정규화·형식오류 1건 메모 보존. dry-run(미리보기) 기본·재실행 안전(멱등). `companies`에 장부번호(구 시스템 대조키)·휴대폰 컬럼 추가.
- **고객 목록 전면 개편(1,270건 대응)** — 한 번에 다 불러오던 방식(1,000건 한계로 일부 누락)을 서버 검색+30건 더보기로 교체. 업체명·사업자번호·장부명·전화·휴대폰 검색, 이름순/최근수정순 정렬, 지역(시·도) 배지, 장부명 부제, 상단 전체/배정/미배정 카운트. 상세·수정 화면에 휴대폰·장부번호 표시.
...
### Fixed
- **의뢰 목록 검색 안정화** — 빠른 타이핑·탭 전환 시 먼저 출발한 느린 응답이 최신 결과를 덮어쓰던 레이스, 요청 실패 시 "불러오는 중…"에 멈추던 문제, 더보기 응답이 갱신된 목록 위에 중복으로 붙던 엣지를 요청 시퀀스 가드로 수정.
- **장비 이미지 삭제 비가역 문제** — 수정 화면에서 기존 이미지를 X로 지운 뒤 저장하지 않고 취소해도 실제 파일이 지워져 공개 카탈로그 이미지가 깨지던 문제. 기존 이미지 제거는 저장 시점에만 반영(미저장 업로드분은 기존대로 즉시 정리).
- **견적서 PDF 링크 만료** — 화면에 박제된 서명URL이 10분 뒤 만료돼 클릭 시 에러가 나던 문제. 클릭 시점에 새 서명URL을 발급하는 전용 라우트(`/admin/quotes/[id]/pdf`, 권한 가드)로 교체 + PDF 생성 폴링의 네트워크 오류 처리.
- 목록 조회 서버 액션 파라미터 검증(음수 offset·거대 limit·임의 scope 거부, 테스트 6건) · 의뢰 상세 조회 에러 로깅(DB 장애가 "찾을 수 없음"으로 위장 방지).
...
### Added
- **견적서 PDF 양식 완성 + 실물 자산** — 재현테크 실제 양식에 맞춰 견적서 PDF 레이아웃을 다듬었다. 글씨 크기 조정, 공급자 박스(페이지 중앙 시작·단일 테두리·라벨 중앙정렬·전화 정렬), 수신처(공급자 박스 좌측 하단·우측정렬·전체 밑줄), 합계금액 띠(라벨 박스 확대·값 우측정렬·회색 배경·단위 굵게), 하단 배너 페이지 바닥 고정, 한글 단어 단위 줄바꿈. 실제 도장(인감) 자산 적용.
- **발행 후 '견적서 확인' 버튼 자동 활성화** — 발행 직후 워커가 PDF를 비동기로 만들므로, 상세 화면이 `pdf_url`을 폴링(2.5s, 최대 60s)해 새로고침 없이 버튼을 활성화한다(`getQuotePdfUrl` 액션 + `QuotePdfButton`).
- **견적 작성/수정 화면 편집 모드 시각 구분** — 상세(읽기) 화면과 헷갈리지 않게 상단 편집 배너 + 편집 영역(장비·옵션·고객 박스) accent 좌측 레일.

### Fixed
- **견적서 PDF 다운로드 깨짐** — 비공개 `quote-pdfs` 버킷의 raw 경로를 href로 써서 다운로드가 "신청서를 찾을 수 없습니다"로 실패하던 문제 → 서버에서 서명URL을 생성해 전달.
- '견적서 발행' 버튼 라벨을 '견적서 확인'으로(발행 동작과 혼동 방지).
...
### Added
- **견적 작성 폼 카탈로그 개편 + 포함옵션 체크박스** — 장비를 자유입력 대신 **카탈로그 드롭다운**에서 선택(이름·기본공급가 자동, "직접 입력" 폴백). 선택 장비의 **포함옵션이 체크박스(기본 전체 포함)**로 떠 관리자가 일부 해제 가능. 선택된 포함옵션은 견적 `options` jsonb에 `kind:'included'`(단가 0)로 **스냅샷 저장** → 금액 영향 없음, 발행본은 카탈로그 변경에도 불변(기존 라이브파생 버그 해결). 수기 견적·재발행도 동일. 견적 프레임 "포함 옵션"은 발행본=스냅샷, 미발행=요청장비 카탈로그에서.
- **견적 작성 시 요청 장비 자동 셋팅** — 새 견적 작성 페이지를 열면 고객이 신청 시 고른 장비(`equipment_id`, 없으면 `equipment_name` 매칭)가 드롭다운에 미리 선택되고 포함옵션이 전체 체크된 채 로딩(담당자 재입력·오선택 방지). 공개 신청 폼은 장비만 수집하므로 추가옵션은 비어 있음.

### Changed
- 견적 줄(`QuoteLine`)에 선택적 `kind`(included/extra) 필드 추가(jsonb·스키마 보존, 계산은 무시). DB 스키마 변경 없음.
