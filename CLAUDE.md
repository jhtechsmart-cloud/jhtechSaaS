...
- **AS 히스토리 데이터 모델(#242 Part 1a)** [세션27 확립]: 모델 단위 집계의 **단일 원본 = `service_reports.catalog_equipment_id`**(확정 시 서버가 해석해 기록, 이후 동결). `company_equipment.equipment_id`는 **자산관리·Part 4 옵션용**이지 통계 원본이 아니다(둘 다 쓰면 반쪽씩 채워져 COALESCE 지옥). 해석 우선순위(**downgrade 금지**) = ①draft에 저장된 피커 선택 id(**존재만 검증, active 불요** — draft 이후 카탈로그가 inactive로 바뀌어도 기사 선택을 지우지 않는다) ②보유장비가 가리키는 카탈로그 ③`match_catalog_equipment(text)` 이름/모델 매칭(active). 매칭 규칙은 **SQL 함수 1벌**로 통일(RPC·마이그레이션 공용, 정규식 복붙 금지) — 다중매칭이면 **null**(오연결보다 미연결이 안전, 프로덕션에 동명 카탈로그 2행 실존: `대형 롤투롤 UV 프린터` = XTRA 5000 / XTRA 3300S). ⚠️ `min(uuid)` 집계는 없다 → `case when count(*)=1 then (array_agg(id))[1] end`.
- **확정 시 보유장비 재사용 = 이력 분할 차단** [세션27]: `issue_service_report`가 `company_equipment`를 무조건 INSERT 하면 카탈로그 피커로 고를 때마다(피커는 `company_equipment_id:null`) 같은 고객·같은 장비 행이 새로 생겨 **이력이 쪼개지고 통계 분모가 팽창**한다. 재사용 조회 3단 = ①시리얼 완전일치(**양쪽 모두 비어있지 않을 때만** — `upsert`가 미입력을 `''`로 저장하므로 가드 없으면 시리얼 없는 장비끼리 오매칭) ②카탈로그 일치(**시리얼 무모순일 때만** — 같은 모델 2대 보유를 한 행으로 병합 금지) ③정규화 이름. `FOR UPDATE`는 리포트 행만 잠그므로 **고객 단위 `pg_advisory_xact_lock`**으로 동시 확정을 직렬화. 재사용 시 **비어 있던 값만 보강**(사람 입력을 기계가 덮지 않는다).
- **장비 상세(#243) = 개요/AS 이력/통계 3탭 허브** [세션28 확립]: 가드 = 리포트 조회 3키(write/view/view_all) ∪ `equipment.manage` — ⚠️ `view_all` 단독 요구 금지(영업 실권한은 `view`, 전원 403). 모델 이력·통계 조회 = **`service_reports.catalog_equipment_id` 직접 + status 앱 명시**(company_equipment 조인이면 영업 RLS로 타 담당 고객분이 조용히 누락, view_all은 draft 혼입). **이력 탭(issued+voided 300)과 통계 탭(issued 전용 301-fetch + voided count 분리)은 별도 조회 계약** — 합치면 최근 무효가 300 표본을 잠식. 미연결 안내 = SECURITY DEFINER 카운트 RPC(match 함수 재사용·내부 권한검사 — 직접 조회는 뷰어마다 다른 숫자). URL 필터 UI = `window.history.replaceState` shallow 갱신(router.replace는 키 입력마다 RSC 재조회+입력 레이스, useListParams 패턴). Next 스트리밍서 notFound()=HTTP 200 → e2e는 문구 단언.
- **집계 순수함수(#244) = 전처리 내재화 계약** [세션28 확립]: `service-stats.ts` 4함수는 voided 필터·null 드롭·그룹 내 ASC 재정렬을 **함수 내부가 책임**(+제외 건수 반환)하고 입력은 모델 무관 `EquipmentReportRow[]` — 횡단(전 모델) 뷰가 그대로 재사용. 월 버킷 = KST **월 앵커** 헬퍼(`periodCutoffKst`는 일 앵커라 부분월 버그), TZ 비의존 unit(KST 자정 ±1h — 로컬 KST·Vercel UTC 괴리) 필수. 발행 리포트는 e2e 시드 불가(BEFORE INSERT 트리거가 service_role에도 draft 강제) → 데이터 케이스는 unit, e2e는 0건 상태만. 블록 차트 = 1블록=1건 비례 금지 하우스 룰.
- **발행본 동결 우회는 status 전환 UPDATE에 합칠 때만** [세션27]: 트리거는 `old.status = 'issued'`일 때만 화이트리스트를 검사하므로 draft→issued 전환 UPDATE에는 새 컬럼을 자유롭게 쓸 수 있다. **확정 후 별도 UPDATE는 예외로 확정 전체가 실패**한다. ⚠️ 화이트리스트에 컬럼을 추가하는 우회 금지 — 앱에서도 발행본이 영구 수정 가능해진다. 이미 발행된 행의 백필은 **마이그레이션에서 트리거를 잠깐 끄고**(exception 경로에서도 반드시 되켤 것) 처리.
- **스토리지 정책 함정 2종** [세션27]: ①**UPDATE 정책이 없으면 `upsert:true`가 거짓 안전** — 첫 업로드는 성공하고 두 번째(재서명·사진 교체)부터 조용히 거부된다. `remove` 후 `upload`로. ②**테이블 RLS만 막으면 첨부로 샌다** — `service-reports` read 정책이 버킷 전체를 조건 없이 허용하고 있어, 읽기전용 권한을 얹으면 영업이 기사의 draft 서명 이미지를 열 수 있었다. 첨부가 있는 도메인은 **테이블 정책과 스토리지 정책을 항상 같이** 보고, 스토리지에도 상태·소유 스코프를 건다.
- **서비스 리포트 조회 권한 3단** [세션27]: `service_reports.write`(현장 작성) / **`service_reports.view`(발행·무효본만 — 영업 기본)** / `service_reports.view_all`(draft 포함 전체). ⚠️ `view_all`은 RLS 세 갈래 중 **draft를 포함하는 유일한 갈래**라 영업에 주면 안 된다(서명 전·금액 미확정 문서 노출). 권한 키 추가 시 **앱 6곳 전수 동기화**: registry·`SALES_PRESET`·`guard.ts`·사이드바(`layout.tsx`)·`email_log_select` 정책·PDF상태 RPC. 기사(`write`)는 `company_equipment` **조회**도 열려 있어야 한다(안 그러면 `/field` 2단계가 항상 빈 목록 → 매번 직접입력).
- **장비→홈페이지(WP) 자동 등록(#253) [세션29 확립]**: 체크박스=`equipment.wp_publish_enabled`(폼 수정 가능한 유일한 wp_* 컬럼), 나머지 `wp_post_id`·`wp_post_status`·`wp_media`(jsonb {path:{id,url}})·`wp_dirty`·`wp_dirty_at`·`wp_last_error`·`wp_synced_at`은 서버 통제 — **`record_wp_sync` DEFINER RPC만이 단일 진입점**(tx-local 플래그는 UPDATE 한 문장 직전 set·직후 clear — tx 끝까지 살면 같은 tx 후속 UPDATE의 가드·enqueue 트리거가 통째로 꺼짐). 반영 정책 = **초안·미생성은 저장 시 자동 sync, 공개 글은 `wp_dirty`만 set → [홈페이지 갱신] 버튼으로만**(오타도 즉시 노출 금지). enqueue = jobs가 RLS 정책 0이라 **AFTER 트리거(INSERT/UPDATE/DELETE)+`enqueue_wp_publish` DEFINER RPC만**(웹 직접 INSERT 불가). 멱등·동시성 = **WP post meta `jhtech_equipment_uuid` 재연결 + 활성 잡 장비당 1건 부분 유니크 + record의 post_id CAS**(⚠️ advisory lock·tx-local 직접 호출은 supabase-js 워커에서 무효 — SQL RPC 트랜잭션 안에서만 성립). unpublish는 sync보다 항상 우선: enqueue 시 queued 대체 + 워커 sync 자가 치유(상태 재확인 후 스스로 내림) + publish 재검증 3중 방어. ⚠️ **WP 코어 REST는 meta_key/meta_value 쿼리를 에러 없이 무시**(최신 글 반환) → findByMeta는 응답 meta를 클라이언트 대조(불일치=미발견=재생성이 오연결보다 안전). 발행기 경계 = `WpPublisher`(Rest: https 강제·30s 타임아웃·permanent/auth/not_found/transient 분류 / Fake: env 3종 미설정 시 실호출 0 — HIWORKS 패턴 동형). dirty 타임스탬프는 `clock_timestamp()`(now()는 tx 상수라 CAS 무력) + 문자열 왕복(pg Date 변환은 μs 소실).
- **RLS 테스트**: vi.mock 불가 → `packages/db-tests`(pg `set role`+`request.jwt.claims`, Supabase 로컬)로 권한별 단언. 순수 로직은 Vitest 단위. ⚠️ **로컬 Supabase 이미지(CLI 2.90) 함정: revoke된 함수를 authenticated/anon이 직접 호출하면 backend segfault**(기존 `claim_next_job`도 재현) — 함수 권한 거부 단언은 실행 대신 `has_function_privilege`로. consumables anon 테스트 등 잔여 실패 4건의 원인(환경 버그, 코드 무관).
...
- **codex = 사용 가능**(2026-07-20 확인: `codex-cli 0.144.6`, GPT-5 응답) → `/autoplan` dual-voice·`/spec` 품질 게이트가 **실제로 돈다**. ⚠️ 이전 기재였던 "이 계정은 gpt-5 미지원이라 자동 스킵"은 **사실이 아니었다** — 실제 원인은 npm 패키지의 플랫폼 바이너리 누락(`@openai/codex-darwin-arm64` vendor ENOENT)이었고 `npm i -g @openai/codex` 재설치로 해결. codex가 `[codex-unavailable]`로 뜨면 미지원으로 단정하지 말고 `codex --version` → 재설치를 먼저 시도할 것.
