...
| 2026-06-13 | **대시보드 날짜 배지 5색 틴트 + 배경 밝힘·그림자 강조** — 일정/신청/활동 행의 날짜·시간 음영 배지를 EVENT_META 5색(견적=민트/파인 · A/S=코랄 · 소모품=라임 · 데모=틸 · 납품=파랑)으로 항목별 구분(`REQUEST_DOMAIN_EVENT` 매핑). 본문 bg `#F8FBFA`→`#FBFDFC`, `shadow-card` 농도 0.05/0.10→0.08/0.16(파인 틴트 유지). 배지 폭=단일시간 목록 `w-20`/시간범위 일정 레일 `w-[88px]`(wide) 고정 — 제목 시작 정렬 유지. | Seonje님 요청(항목 시각 구분 + 밝은 배경 + 그림자 강조). 색 단일 출처 = v2-meta.ts EVENT_META. |
